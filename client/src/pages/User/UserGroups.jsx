...
const UserGroups = () => {
    ...
    return (
        ...
        >
            <motion.h1
                className="text-3xl  mt-8  font-mono"
            ...
            <motion.div className="flex flex-wrap -mx-4" variants={cardVariants}>
                {user?.confessionGroups?.map((group) => (
                    <UserGroupCard key={group._id} group={group}  />
                ))}
            </motion.div>
            {user?.confessionGroups.length === 0 && (
                <motion.div variants={messageVariants} className="w-full text-center text-red-300 font-black text-lg">
